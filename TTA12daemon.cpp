			...
			for( AIndex = 0 ; AIndex < MAX_ACTUATOR_COUNT ; AIndex++) //액츄에이터 ON TIME 처리 
			{
				...
				if( AI[AIndex].data.value > 0  )
				{
						...
						{ 
							...
							printf( "Actuator OFF index:%d id:%d (%d)\n", AIndex,AI[AIndex].data.id,AI[AIndex].data.value) ;						
				...
				if( LiveTime > 0 ) // ON 상태의 액츄에이터 체크
				{
					...
					if( ACTUATOR_MAP[	SIndex ].COUNT > 0 )
					{
							...
							if( ACTUATOR_MAP[	SIndex ].COUNT <= 0 )
							{
									...
									printf("Actuator On Status Transfer time is up : ID: %d,%d\n",AI[SIndex].data.id ,AI[SIndex].data.value) ; 
									if( TransActuatorDataCount < 63 ) TransActuatorDataCount++ ;
									// 엑추에이터 타임카운트 초기화 
	    ...
	    int nodeDelay=0;
			for( int bs = 0 ; bs < InstalledSensorNodeCount ;bs++)
			{
				...
				if( SENSOR_MAP[bs*8].TYPE == 1 ) // AD BOARD
				{
					...
					USART_SendPacket(CONTROL_PORT, &Packet ) ; usleep(1000*50) ;nodeDelay++;
...
void *RS232_Thread(void * arg)
{
	...
	while(1)
	{
			...
			if( INSTALL_COMM_SENSOR_COUNT >= 1 )
			{
				n = RS232_PollComport(COMM_SENSOR[0].PORT, readbuf, 254);
				if(n > 0)
				{
					pthread_mutex_lock(&mutex_rsbuf1);
					for( cnt = 0 ; cnt < n ;cnt++) 
					{
						if(COMM_SENSOR[0].PORT==2) USART2_RxQuePush( readbuf[cnt] ) ; 
						if(COMM_SENSOR[0].PORT==3) USART3_RxQuePush( readbuf[cnt] ) ; 
						//printf("comm2 thread %x %x \n",readbuf[cnt],USART2RxDataQue.nBuffer[USART2RxDataQue.nPushIndex-1] ) ; 
					}
					memset( readbuf,0x00,255) ;
					pthread_mutex_unlock(&mutex_rsbuf1);
				}
			...
			if( INSTALL_COMM_SENSOR_COUNT >= 2 )
			{
				n = RS232_PollComport(COMM_SENSOR[1].PORT, readbuf, 254);
				if(n > 0)
				{
					pthread_mutex_lock(&mutex_rsbuf1);
					for( cnt = 0 ; cnt < n ;cnt++) 
					{
						if(COMM_SENSOR[1].PORT==2) USART2_RxQuePush( readbuf[cnt] ) ; 
						if(COMM_SENSOR[1].PORT==3) USART3_RxQuePush( readbuf[cnt] ) ; 	
					}
					memset( readbuf,0x00,255) ;
					pthread_mutex_unlock(&mutex_rsbuf1);
				}
					...
					{  
						...
						if( nodeCH > -1 )
						{
							if( aVALUE > 0 )
							{
								...
								ACTUATOR_MAP[	nodeCH ].COUNT= aVALUE ;//ActuatorInterval ;
								...
								printf("ON [id=%d,ARG=%X] findINDEX=%d,Delay %d \n",TTAParser.aData[i].id,aARG,nodeCH,ACTUATOR_MAP[	nodeCH ].COUNT ) ;
							} else {
								
								printf("OFF [id=%d,ARG=%X] findINDEX=%d,Delay %d \n",TTAParser.aData[i].id,aARG,nodeCH,ACTUATOR_MAP[	nodeCH ].COUNT ) ;
...
bool UART2SensorPacketStreaming( void )
{
	u8					nch, i;
	u16					nchksum;
  ...
  //printf("SensorPacketStreaming  %x \n", USART2RxDataQue.nBuffer[USART2RxDataQue.nPushIndex-1] ) ; 
	while( UARTFifoPop(&USART2RxDataQue, &nch) ) 
	{
		// packet streaming
		...
		if( nch == 0x0A || nch == 0x0D ) newlineflag1++ ; // newline 데이터가 한번이라도 입력된후 숫자 데이터 입력
		
		if( newlineflag1 > 0 )
		{
			if( nch == 0x0A || nch == 0x0D ) break; 
			
			USART2Rxbuf[ comm_data_count1++]=nch ;
			if( comm_data_count1 >= COMM_SENSOR[0].LEN ) 
			{ 
				newlineflag1=0;
				comm_data_count1=0;
				//통신센서로 부터 헤더 데이터가 있는지 확인 필요
				COMM_SENSOR_VALUE[0] = atoi( (char *)USART2Rxbuf ) ; 
				printf( "COMM2 sensor data %s %d\n",USART2Rxbuf,COMM_SENSOR_VALUE[0]) ;
				return true;
...
bool UART3SensorPacketStreaming( void )
{
	u8					nch, i;
	u16					nchksum;
	//u8					*buffer = (u8*)&USART3Rxbuf;
	
  while( UARTFifoPop(&USART3RxDataQue, &nch) ) {
	
		if( nch == 0x0A || nch == 0x0D ) newlineflag2++ ; // newline 데이터가 한번이라도 입력된후 숫자 데이터 입력
		
		if( newlineflag2 > 0 )
		{
			if( nch == 0x0A || nch == 0x0D ) break; 
					
			USART3Rxbuf[ comm_data_count2++]=nch ;
			if( comm_data_count2 >= COMM_SENSOR[1].LEN ) 
			{	 
				newlineflag2=0;
				comm_data_count2=0;
				//통신센서로 부터 헤더 데이터가 있는지 확인 필요
				COMM_SENSOR_VALUE[1] = atoi( (char *)USART3Rxbuf ) ; 
				printf( "COMM3 sensor data %s %d\n",USART3Rxbuf,COMM_SENSOR_VALUE[1]) ;
				return true;
...
void SensorValueReadToNode (void)
{
		...
		NodeID = SearchIndexSensorNode(USART1StreamingBuffer.wCanID) ; 
		...
		if( NodeTYPE == 0x00 ) // BASE board 단동모델제어시
		{
			...
			{			
				...
				printf( "BASE COUNTER  [%04x : %d] IN CH1 %d,CH2 %d\n",
											USART1StreamingBuffer.wCanID,
						SearchIndexSensorNode(USART1StreamingBuffer.wCanID),
...
int main(int argc, char **argv)
{
  ...
  if( INSTALL_COMM_SENSOR_COUNT >= 1 )
	{
		printf("Serial port #2 open for COMM SENSOR( RS485 )\n") ;
		if( RS232_OpenComport(COMM_SENSOR[0].PORT, COMM_SENSOR[0].SPEED) )
		{
			printf("Can not open serial port #2 for COMM SENSOR (RS485)\n") ;
		}
	}
  if( INSTALL_COMM_SENSOR_COUNT >= 2 )
	{
		printf("Serial port #3 open  for COMM SENSOR( RS323 )\n") ;
		if( RS232_OpenComport(COMM_SENSOR[1].PORT, COMM_SENSOR[1].SPEED) )
		{
			printf("Can not open serial port #3 for COMM SENSOR( RS232 )\n") ;
		}	
	}
	RS485Ctrl_flow(0) ;  // receive mode

	memset( USART2Rxbuf,0,10 ) ; 
	memset( USART3Rxbuf,0,10 ) ; 
	...
	printf("======================================\n") ;
	printf("TTA1,2 daemon build 3.1001  2016-5-28  \n" ) ;
	...
	while(1)
	{
		...
		if( UART2SensorPacketStreaming() )
		{
				SI[COMM_SENSOR[0].SENSOR_NODE*8+0].data.value = COMM_SENSOR_VALUE[0] ; 
		}
		if( UART3SensorPacketStreaming() )
		{
				SI[COMM_SENSOR[1].SENSOR_NODE*8+0].data.value = COMM_SENSOR_VALUE[1] ; 
		}
