	    ...
	    for( int bs = 0 ; bs < InstalledSensorNodeCount ;bs++)
			{
				...
				if( SENSOR_MAP[bs*8].TYPE == 1 ) // AD BOARD
				{
					...
					USART_SendPacket(CONTROL_PORT, &Packet ) ; usleep(1000*100) ;
...
int main(int argc, char **argv)
{
  ...
  ReadServer_Config() ;	
	...
	printf("======================================\n") ;
	printf("TTA1,2 TEST PROGRAM build 1.0001  2016-3-22  \n" ) ;
	...
	while(1)
	{
		...
		if( TCP_CMD_HEAD < TCP_CMD_TAIL )
		{
			...
			printf("Reveived CMD Packet...................................................\n") ;
			HexaPrint(  TCP_CMD_BUF[TCP_CMD_HEAD].buf,TCP_CMD_BUF[TCP_CMD_HEAD].size ) ;
			
			pthread_mutex_lock(&cmd_mutex);
			if( TCP_CMD_BUF[TCP_CMD_HEAD].size >= COMMPACKET_SIZE )
			{
				...
				//
				//  컴퓨터에서 받은 TCP/IP 패킷을 ARM으로 직접 송출( RS232 ) 
				//
				
				for( idx=0; idx < COMMPACKET_SIZE ;idx++)
				{
					((BYTE *)&mPacket)[idx]=TCP_CMD_BUF[TCP_CMD_HEAD].buf[idx] ; 
				}

				USART_SendPacket(CONTROL_PORT, &mPacket ) ;usleep(1000*100) ;
...
int main(int argc, char **argv) {
       ...
       while((ret = regexec(&reg, str+offset, 1, &pmatch, eflags))
== 0)
       {
